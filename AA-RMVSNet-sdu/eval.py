...
def save_depth():
    ...
    test_dataset = MVSDataset(args.testpath, args.testlist, "test", 10, args.numdepth, args.interval_scale, args.inverse_depth, 
                    adaptive_scaling=True, max_h=args.max_h, max_w=args.max_w, sample_scale=1, base_image_size=1)
    ...
    print("loading model {}".format(args.loadckpt), flush=True)
    ...
    if "module.feature.conv0_0.0.weight" in state_dict['model']:
        print("With module in keys", flush=True)
        model = nn.DataParallel(model)
        model.load_state_dict(state_dict['model'],True)
        
    else:
        print("No module in keys", flush=True)
        model.load_state_dict(state_dict['model'], True)
        model = nn.DataParallel(model)
    model.cuda()
    model.eval()
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            if count <= 9:
                continue
            '''
            print('process', sample['filename'], flush=True)
            sample_cuda = tocuda(sample)
            print('input shape: ', sample_cuda["imgs"].shape, sample_cuda["proj_matrices"].shape, sample_cuda["depth_values"].shape, flush=True)
            time_s = time.time()
            outputs = model(sample_cuda["imgs"], sample_cuda["proj_matrices"], sample_cuda["depth_values"])

            one_time = time.time() - time_s
            total_time += one_time
            print('one forward: ', one_time, flush=True)
            if count % 50 == 0:
                print('avg time:', total_time / 50, flush=True) 
                total_time = 0

            outputs = tensor2numpy(outputs)
            del sample_cuda
            print('Iter {}/{}'.format(batch_idx, len(TestImgLoader)), flush=True)
            ...
            for filename, depth_est, photometric_confidence in zip(filenames, outputs["depth"],
                ...
                # save depth maps
                print(depth_est.shape, flush=True)
